...
class allsparkModePanel(wx.Panel):
	"Main user interface window for Allspark mode"
	def __init__(self, parent, callback):
		super(allsparkModePanel, self).__init__(parent)
		...
		for filename in resources.getAllsparkModeProfiles():
			...
			if profile.getPreference('allsparkModeProfile') == base_filename:
				button.SetValue(True)

		printMaterialPanel = wx.Panel(self)
		for filename in resources.getAllsparkModeMaterials():
			cp = configparser.ConfigParser()
			cp.read(filename)
			base_filename = os.path.splitext(os.path.basename(filename))[0]
			name = base_filename
			...
			if profile.getPreference('allsparkModeMaterial') == base_filename:
				button.SetValue(True)
	...
	def _update(self, e):
		for button in self._print_profile_options:
			if button.GetValue():
				profile.putPreference('allsparkModeProfile', button.base_filename)
		for button in self._print_material_options:
			if button.GetValue():
				profile.putPreference('allsparkModeMaterial', button.base_filename)
		self._callback()
