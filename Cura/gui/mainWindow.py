...
from Cura.gui import aboutWindow, alterationPanel, configBase, configWizard, \
    expertConfig, firmwareInstall, pluginPanel, preferencesDialog, sceneView, \
    simpleMode, allsparkMode
from Cura.gui.tools import minecraftImport, pidDebugger
from Cura.gui.util import dropTarget
from Cura.util import meshLoader, profile, version
...
class mainWindow(wx.Frame):
    def __init__(self):
        ...
        expertMenu = wx.Menu()
        i = expertMenu.Append(-1, _("Switch to allspark..."), kind=wx.ITEM_RADIO)
        self.switchToAllsparkMenuItem = i
        self.Bind(wx.EVT_MENU, self.OnAllsparkSwitch, i)
        ...
        ##Gui components##
        self.allsparkSettingsPanel = allsparkMode.allsparkModePanel(self.leftPane, self.scene.sceneUpdated)
        ...
        self.leftSizer.Add(self.allsparkSettingsPanel, 1)
        ...
        self.allsparkSettingsPanel.Show(False)
    ...
    def updateSliceMode(self):
        isAllspark = profile.getPreference('startMode') == 'Allspark'
        isSimple = profile.getPreference('startMode') == 'Simple'

        self.normalSettingsPanel.Show(not isSimple)
        self.allsparkSettingsPanel.Show(isAllspark)
        ...
        if isAllspark:
            self.switchToAllsparkMenuItem.Check()
        elif isSimple:
        ...
        if isAllspark:
            ...
            x = self.splitter.GetSashPosition()
            isSmallSashPosition = x <= 250
            if isSmallSashPosition:
                self.normalSashPos = self.normalSashPos
            else:
                self.normalSashPos = self.splitter.GetSashPosition()
                
            # Change location of sash to width of allspark mode pane
            (width, height) = self.allsparkSettingsPanel.GetSizer().GetSize()
    ...
    def OnAllsparkSwitch(self, e):
        profile.putPreference('startMode', 'Allspark')
        self.updateSliceMode()
